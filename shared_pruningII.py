"""
    ...
    python -u shared_pruningII.py --data ../data --dataset cifar100 --arch resnet18 --seed 1 --rate 0.9 --save_dir ./runs/sharedII_seed1_resent18_cifar100_rate90 > ./runs/sharedII_seed1_resent18_cifar100_rate90/shared_run1.txt
"""
import os
import sys
import pdb
...
def main():
    global args, best_sa, fopen, test_loader
...
def train(train_loader, model, optimizer, criterion, mask90, epoch):
    ...
    mask_rate = prune_and_extract(model, rate)
    remove_prune(model)
    ...
    for i, (image, target) in tqdm(enumerate(train_loader)):
        ...
        remove_prune(model)
        model_copy = deep_copy(model)
        prune_model_custom(model, mask_rate)
        output = model(image)
